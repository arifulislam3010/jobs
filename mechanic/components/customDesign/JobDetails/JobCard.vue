      ...
      <div class="job-card-header">
        <div class="job-title">
          <div class="logo-wrapper">
            <img :src="post.image" alt="job logo" class="img-fluid" />
          </div>
        ...
        <div class="save-badge" v-if="$auth.user && $auth.user.type != 3">
          <button
            v-if="check.favourite"
            @click="removeTofavourite()"
            style="width:94px"
          >
            <i class="far fa-bookmark"></i>Remove
          </button>
          <button v-else @click="addTofavourite()">
    ...
    <div class="job-card-footer ">
      ...
      >
        <div class="job-collapse-button">
          <b-button v-b-toggle="'collapse-' + post.id" variant="link"
      ...
      <div class="job-card-main-content-wrapper" id="collapseExample">
        <div class="job-card-main-content  d-flex justify-content-between">
          ...
          <div class="card-expand-close-button-wrapper">
            ...
            >
            <button v-if="check.applyied > 0" class="btn  btn-success">
              Applied
            </button>
            <button
              v-if="check.applyied == 0 && loader == true"
              class="btn  btn-success"
            >
              <i class="fa fa-cog fa-spin"></i> Appling
            </button>
            <button
              v-if="
                check.applyied == 0 &&
        ...
        <div class="job-description">
          ...
          <div
            style="display: flex;flex-direction: column;min-width: 0;word-wrap: break-word;"
          >
...
export default {
  ...
  methods: {
    ...
    async addTofavourite() {
      if (this.$auth.loggedIn) {
        await this.$axios
          .get('api/add/favourite/' + this.post.id)
          .then(response => {
    ...
    async removeTofavourite() {
      if (this.$auth.loggedIn) {
        await this.$axios
          .get('api/remove/favourite/' + this.post.id)
          .then(response => {
    ...
    async checkApply() {
      await this.$axios
        .get('api/check-apply/' + this.post.slug)
        .then(response => {
